...
crabs2 <- fmutate(crabs, group  = labelise(
...
chart(data = crabs2, rear ~ group) +
  ...
  geom_point(data = fgroup_by(crabs2, group) |> fsummarise(
    means = mean(rear, na.rm = TRUE)),
...
N'oublions pas que, à chaque test, nous prenons un risque de nous tromper. **Le risque de se tromper au moins une fois dans l'ensemble des tests est alors décuplé en cas de tests multiples.** Prenons un point de vue naïf, mais qui suffira ici pour démontrer le problème qui apparaît. Admettons que le risque de nous tromper est constant, que nous rejetons ou non $H_0$, et qu'il est de l'ordre de 10% dans chaque test individuellement[^10-variance-i-1]. La seule solution acceptable est que *tous* les tests soient corrects. Considérant chaque interprétation indépendante, nous pouvons multiplier les probabilités d'avoir un test correct (90%) le nombre de fois que nous faisons le test, soit $0,9 \times 0,9 \times 0,9 \times 0,9 \times 0,9 \times 0,9 = 0,9^6 = 0,53$. Tous les autres cas ayant au moins un test faux, nous constatons que notre analyse globale sera incorrecte $1 - 0,53 = 47\%$ du temps[^10-variance-i-2]. **Notre analyse sera incorrecte une fois sur deux environ.**
...
`r h5p(229, height = 270, toc = "Comparaison 2 à 2 et erreurs associées")`
...
pl1 <- chart(data = dat1, Y ~ Population) +
  ...
  geom_point(data = fgroup_by(dat1, Population) |> fsummarise(
...
pl2 <- chart(data = dat2, Y ~ Population) +
  geom_boxplot() +
  geom_jitter(width = 0.05) +
  geom_point(data = fgroup_by(dat2, Population) |> fsummarise(
    means = mean(Y, na.rm = TRUE)),
    f_aes(means ~ Population), size = 3, col = "red") +
  ylim(-1.5, 1.5)
...
[^10-variance-i-3]: Les fonctions qui permettent les calculs relatifs à la distribution *F* dans R sont `<x>f()`, et les snippets correspondants dans la SciViews Box sont disponibles à partir de `.if`. Leur utilisation est similaire à celle des distributions vues au module \@ref(proba).
...
```{r}
crabs2 <- fmutate(crabs2, log_aspect = ln(aspect))
...
```{r}
crabs2 <- fmutate(crabs2, aspect5 = aspect^5)
...
Nous poursuivons sur une description des données utile pour l'ANOVA[^10-variance-i-4] :

[^10-variance-i-4]: Un snippet dédié est disponible dans le menu `hypothesis tests: means` à partir de `.hm`.

```{r}
crabs2 %>.%
  fgroup_by(., group) |> fsummarise(
    mean  = mean(aspect5),
    sd    = sd(aspect5),
