...
\end{equation}
```
A noter que $s^2$ est également appelée la **variance**[^09-moyenne-2].
...
Nous pouvons également employer les objet distribution afin de répondre à cette question (et les questions suivantes)

```{r}
stu <- dist_student_t(df = 8, mu = 8, sigma = 2/3)
1 - cdf(stu, 8.5)
```
...
(left_area <- cdf(stu, 7.5))
# Aire à droite de 8 + 0.5 :
(right_area <- 1 - cdf(stu, 8.5))
# Résultat final
...
```{r}
# Quantile à gauche
quantile(stu, p = 0.025)

# Quantile à droite
quantile(stu, p = (1-0.025))
```
