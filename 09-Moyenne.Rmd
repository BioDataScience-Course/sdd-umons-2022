...
<!-- Here, we have to move functions from BioDataScience1 to inferit. We need also to look at a way to estimate best n given a goal. Work on effect size and absolute effect in a way that it would be homogeneous with those of Chi^2 and ANOVA. We also need a nicer presentation of output in reportit + labels, incl. in french. A Shiny app to feel the various aspects presented here. Also a supplemental section that presents bootstrap and bootstrapped t test + a Bayeasian alternative. -->
...
[^09-moyenne-1]: Vous pouvez activer les sous-titres en anglais via la barre de boutons en bas de la vidéo pour vous aider à comprendre l'histoire.
...
Concernant la moyenne, et l'écart type, nous pouvons les calculer sur base de notre distribution d'échantillonnage empirique contenue dans le vecteur `means_n9` :
...
La moyenne de la distribution d'échantillonnage est donc égale à la moyenne de la population. Elle peut être approximée par la moyenne d'un échantillon. Quant à l'écart type, il vaut 2/3 environ, soit l'écart type de la population divisé par 3.
...
La distribution est légèrement plus étalée (il faut bien observer pour le voir). Ses paramètres sont :
...
La moyenne de l'échantillon suit une distribution *t* de Student avec pour moyenne, la moyenne de la population, pour écart type, l'écart type de la population divisé par la racine carrée de *n*, et comme degrés de liberté *n* moins un. La distribution *t* de Student dans R est représentée par `<p|q|d|r>t()`. Donc, `qt()` calcule un quantile à partir d'une probabilité, `pt()` une probabilité à partir d'un quantile, `rt()` renvoie un ou plusieurs nombres pseudo-aléatoires selon une distribution *t*, et `dt()` renvoie la densité de probabilité de la distribution.
...
Dans ce cas, il n'est pas nécessaire de ruser car cet objet permet de traiter une distribution de Student non centrée sur 0. Rappelez vous cependant que les fonctions `cdf()` et `quantile()` ne permettent pas de spécifier que l'on souhaite l'aire à droite.
...
Dans ce cas-ci, l'utilisation de `dist_student_t()` simplifie considérablement les calculs... mais `<p|q|r|d>t()` restent plus concis si votre calcul concerne la distribution *t* de Student réduite. Le graphique correspondant au dernier calcul est le suivant :
...
Nous pouvons voir sur la Fig. \@ref(fig:tdistri1) que la distribution *t* de Student est plus resserrée en son centre, mais plus étalée aux extrémités que la distribution Normale de même moyenne et écart type. Néanmoins, elle est d'autant plus proche d'une Normale que les degrés de liberté sont grands. On dit qu'elle converge vers une Normale lorsque $dll \longrightarrow \infty$. En pratique, pour des degrés de liberté égaux ou supérieurs à 30, nous pourrons considérer que les deux distributions sont pratiquement confondues.
...
Vous avez remarqué quelque chose de particulier ? Oui, les deux aires à gauche et à droite sont identiques. C'est parce que la distribution est symétrique. On peut donc simplifier le calcul en calculant d'un seul côté et en multipliant le résultat par deux :
...
Jusqu'ici, nous avons considéré une population au départ qui a une distribution Normale, mais rien ne dit que ce soit le cas. Que se passe-t-il lorsque la distribution est différente ? Ici encore, nous pouvons effectuer une métaexpérience. Considérons, par exemple, une distribution uniforme de même moyenne = 8 et écart type = 2. Sachant que l'écart type d'une distribution uniforme vaut $\frac{max - min}{\sqrt{12}}$, voir [ici](https://fr.wikiversity.org/wiki/Variables_aléatoires_continues/Loi_uniforme), l'intervalle est de : $2 \cdot \sqrt{12} = 6,928$. Nous avons donc :
...
df <- 3
x <- seq(-4.5 * s + mu, 4.5 * s + mu, l = 1000) # Quantiles
student_t1 <- dist_student_t(df = df, mu = mu, sigma = s)
chart(data = NULL, ~ m_unif_n4) +
  geom_histogram(bins = 30) +
  geom_line(aes(x = x, y = dfun(student_t1)(x) * 3000))
```
...
df <- 8
x <- seq(-4.5 * s + mu, 4.5 * s + mu, l = 1000) # Quantiles
student_t2 <- dist_student_t(df = df, mu = mu, sigma = s)
chart(data = NULL, ~ m_unif_n9) +
  geom_histogram(bins = 30) +
  geom_line(aes(x = x, y = dfun(student_t2)(x) * 2000))
```
...
df <- 99
x <- seq(-4.5 * s + mu, 4.5 * s +  mu, l = 1000) # Quantiles
student_t3 <- dist_student_t(df = df, mu = mu, sigma = s)
chart(data = NULL, ~ m_unif_n100) +
  geom_histogram(bins = 30) +
  geom_line(aes(x = x, y = dfun(student_t3)(x) * 610))
```

Nous venons de montrer de manière empirique que lorsque la distribution de la population est différente d'une distribution Normale, la distribution d'échantillonnage tend vers une *t* de Student pour un $n$ grand (en tous les cas, dans le cas d'une distribution de la population uniforme). Ceci se démontre de manière mathématique par le fameux **théorème central limite** que nous avons déjà abordé et qui est si cher aux statisticiens (nous vous épargnons cette démonstration ici).
...
[^09-moyenne-5]: Il existe des tests pour le vérifier, comme le **test de Bartlett**, mais ce n'est pas le propos ici.
...
[^09-moyenne-6]: Pouvez-vous expliquer pourquoi ? Réfléchissez, c'est assez évident.
...
Le graphique suggère peut-être que les patient dorment plus, mais la boite de dispersion chevauche quand même le trait horizontal à zéro. Donc, qu'en est-il exactement ? Le test s'obtient toujours à partir de la fonction `t.test()`. Notez l'argument supplémentaire `mu =` qui permet de spécifier la valeur de référence (ici zéro). Fixons $\alpha$ avant d'effectuer le test. Ici, nous prenons toujours 5%.
...
if (exists("assignment"))
  assignment("A09Ia_ttest", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-027' = "https://classroom.github.com/a/JnOVcRGe"),
    course.starts = c(
...
Pour les données relatives à `med1` de `sleep2`, cela donne[^09-moyenne-7] :

[^09-moyenne-7]: Dans le cas d'un test de Wilcoxon apparié, on a $H_0: mediane_x = mediane_y$.
...
`r learnr("A09Lb_ttest_wmw", title = "Tests t de Student et de Wilcoxon-Mann-Whitney", toc = "Tests t de Student et de Wilcoxon-Mann-Whitney")`
...
if (exists("assignment2"))
  assignment2("A09Ga_human_health", part = "II",
    ...
    course.urls = c(
      'S-BIOG-027' = "https://classroom.github.com/a/wxbGLeOk"),
    course.starts = c(
      'S-BIOG-027' = !"{W[27]+1} 13:00:00"),
    course.ends = c(
