...
-   Comprendre et utiliser la boîte de dispersion pour *synthétiser* la distribution de données numériques
...
Notez aussi que nous utilisons l'opératezur d'assignation alternatif `%<-%` qui permet d'obtenir un tableau comme résultat de `filter()` (sinon un objet est créé qui contient les informations pour effectuer le traitement, mais pas le *résultat* du traitement lui-même).
...
###### Rotation des axes du graphique en barre {.unnumbered}
