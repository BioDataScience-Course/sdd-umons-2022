...
Dans ce cas, un test positif n'aura effectivement détecté un malade que dans ... 7,1% des cas ! Les 92,9% d’autres cas positifs seront en fait des personnes saines.
...
-   Quelle est la probabilité d'avoir un revenu faible ou d'avoir un élevé ? Cette question peut s'écrire : $\mathrm{P}(revenu\-faible\, ou\, revenu\_eleve)$.
...
Mais comme nous voulons déterminer la probabilité qu'un insecte arrive entre 1 et 1.5 minute, nous devons soustraire à cette valeur la probabilité qu'un insecte arrive en moins de 1 min (la zone hachurée en rouge dans la Fig \@ref(fig:unif3) est en effet l'aire à gauche depuis le quantile 1.5 *moins* l'aire à gauche depuis le quantile 1) :
...
La réponse est 0.125, soit une fois sur huit observations, en moyenne, le temps d'attente enregistré sera compris entre 1 et 1.5 minute.
...
Les calculs sur base d'une distribution binomiale sont assez similaires à ceux de la distribution uniforme dans R, en remplaçant `unif` par `binom` dans le nom des fonctions. Voici la liste des snippets à votre disposition dans la SciViews Box pour vous aider (menu `(d)istributions: binomial` à partir de `.ib`) :
...
... soit un couple sur 400. Donc, globalement, les probabilités d'avoir des enfants sains sont beaucoup plus grandes que 0.75 si nous incluons tous les couples belges de parents sains porteurs ou non. Cette probabilité est de[^07-probabilites-distributions-5] :

[^07-probabilites-distributions-5]: Sur 400 couples, 399 ont une probabilité d'engendrer des enfants sains (porteurs hétérozygotes ou non porteurs confondus) de 100%. À cela il faut ajouter un couple sur 400 qui aura une probabilité de 75% de faire des enfants non atteints, car non homozygotes.
