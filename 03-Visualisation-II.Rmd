...
La fin de ce module va traiter de la gestion des conflits dans GitHub. Vous devez maîtriser GitHub et Git.
...
Vous souhaitez visualiser l'étalement de vos données sur un axe (on parle de **distribution**[^03-visualisation-ii-1] en statistique) pour l'une des variables étudiées. L'histogramme est l'un des outils pouvant vous apporter cette information. Ce graphique représente sous forme de barres un découpage en plusieurs **classes**[^03-visualisation-ii-2] d'une variable numérique.
...
-   Les axes avec les graduations (en rouge). Sur l'axe des abscisses, les classes de taille, et sur l'axe des ordonnées, les effectifs
...
Les variables numériques sont représentées par des nombres, donc `numeric` (des nombre décimaux à virgule flottante) ou `integer` (des entiers) dans R. Attention que les variables facteur peuvent très bien être importées comme chaînes de caractères (objet `character`), et il faudra peut-être les convertir à l'aide des fonctions `factor()` ou `ordered()` avant de les utiliser.
...
L'histogramme est maintenant séparé en trois en fonction des niveaux de la variable facteur `species`. Cela rend la lecture plus aisée. Une seconde solution combine les facettes avec `|` et l'argument `%fill=%`[^03-visualisation-ii-5]. Il faut ensuite ajouter par-derrière un histogramme grisé de l'ensemble des données.

[^03-visualisation-ii-5]: Astuce proposée [ici](https://drsimonj.svbtle.com/plotting-background-data-for-groups-with-ggplot2).
...
L'histogramme n'est pas le seul outil à votre disposition. Vous pouvez également utiliser le **graphique de densité** qui se présente un peu comme un histogramme lissé. Le passage d'un histogramme vers un graphe de densité se base sur une **estimation par noyau gaussien**[^03-visualisation-ii-6]

[^03-visualisation-ii-6]: L'opération effectuée pour passer d'un histogramme à une courbe de densité consiste effectivement à lisser les pics plus ou moins fortement dans l'histogramme de départ.
...
if (exists("assignment"))
  assignment("A03Ia_distributions", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-006' = "https://classroom.github.com/a/p0skoF-c"),
    course.starts = c(
      'S-BIOG-006' = !"{W[8]+1} 13:00:00"),
    course.ends = c(
      'S-BIOG-006' = !"{W[8]+1} 16:00:00"),
    term = "Q1", level = 3,
...
Les deux collaborateurs vont pouvoir travailler sur le même document et progresser ensemble dans l'analyse du jeu de données sur la biométrie humaine. En cliquant sur le dernier "commit" réalisé, on peut retrouver la succession des commits qui ont été faits jusqu'ici (historique).
...
```{block, type='note'}
Lorsque vous vous lancez dans un projet ou revenez vers lui, il faut toujours faire un "pull" avant de commencer à travailler afin de vous assurer que vous éditez bien avec la dernière version en ligne sur GitHub. 
...
if (exists("assignment2"))
  assignment2("A03Ga_urchin", part = "I",
    ...
    course.urls = c(
      'S-BIOG-006' = "https://classroom.github.com/a/ewTVBC8e"),
    course.starts = c(
      'S-BIOG-006' = !"{W[8]+5} 08:00:00"),
    course.ends = c(
