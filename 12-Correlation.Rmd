...
ans_y %>.%
  purrr::map_dfr(., mean_var) %>.%
  fmutate(., variable = names(ans_y)) %>.%
  knitr::kable(., col.names = c("Moyenne", "Variance", "Ecart-type", "Variable"))
```

À nouveau, c'est les mêmes valeurs pour les 4 séries. Que donnent les coefficients de corrélation[^12-correlation-1] ?
...
**Finalisez maintenant votre gros projet transmodule sur `urchin` en y ajoutant un test de corrélation et en terminant le rapport.**
...
Hans Rosling est sans nul doute très doué pour communiquer des résultats statistiques. La vidéo suivante est un peu longue (20min) et en anglais, mais elle en vaut vraiment la peine[^12-correlation-2]. De plus, il explique à quel point il est important de partager et de rassembler les données dans de grandes bases de données, et ensuite d'en tirer des études *utiles* pour l'humanité. C'est l'avenir des sciences des données, y compris en biologie, qu'il est en train de prédire là.
...
-   N'y a-t-il pas pseudoréplication (plusieurs mesures issues d'un même individu considérées comme des observations indépendantes) ?
