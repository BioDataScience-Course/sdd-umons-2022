...
[^05-importation-transformation-1]: R permet également d'interroger des bases de données spécialisées, mais nous n'aborderons ce sujet spécifique qu'au cours de Science des Données Biologiques 4 en Master 2.
...
```{r}
urchin_FR %>.%
  fsubset(., origin == "Pêcherie") %>.%
  head(.)
```

Le code a dû être modifié dans l'instruction `filter()` lors du passage à `urchin_FR` (`Farm` -\> `Pêcherie`). Bien évidemment, pour un rapport *plus formel* en français, **tout** doit être traduit en français et l'option `lang = "FR"` accompagnée d'une vérification et une adaptation éventuelle du code est à préférer dans ce cas précis.
...
-   sélectionner des colonnes au sein d'un jeu de données avec `select()`/`fselect()`

-   filtrer des lignes dans un jeu de données avec `filter()`/`fsubset()`

-   calculer de nouvelles variables dans un jeu de données avec `mutate()`/`fmutate()`

-   regrouper les données au sein d'un tableau avec `group_by()`/`fgroup_by()`

-   résumer les variables d'un jeu de données avec `summarise()`/`fsummarise()`
...
Lors de l'utilisation de vos jeux de données, vous serez amené à réduire vos données en sous-tableau ne reprenant qu'un sous-ensemble des variables initiales. `select()` effectue cette opération[^05-importation-transformation-4] :

[^05-importation-transformation-4]: Voyez `?select_helpers` pour une panoplie de fonctions supplémentaires qui permettent une sélection "intelligente" des variables.

La fonction `select()` est une fonction du "tidyverse" (nous l'appelerons désormais une **fonction tidy**) qui a un comportement particulier dans R. Une fonction relativement équivalente, mais "non-tidy" est `fselect()`. Ces deux fonctions sont souvent interchageables, mais pas toujours.

Avec les fonctions tidy, il faut prendre soin de **collecter** les résultats à la fin si on travaille avec des objets de classe **data.table** comme c'est le cas par défaut avec `SciViews::R` (le package {dtplyr} gère alors le calcul). Dans `SciViews::R`, on utilisera l'**assignation alternative** `%<-%` ou `%->%`, ou alors, on utilisera explicitement `collect_dtx()` pour récupérer un data frame utilisable avec les fonctions non-tidy :

```{r}
urchin1 %<-% select(urchin, origin, solid_parts, test) # assignation alternative
rmarkdown::paged_table(urchin1)
```

ou encore (mais c'est plus lourd ici) :

```{r}
urchin1 <- select(urchin, origin, solid_parts, test) |> collect_dtx()
rmarkdown::paged_table(urchin1)
```

Avec la forme non-tidy `fselect()`, ces précautions ne sont pas nécessaires. Toutefois, comme nous le verrons juste après, la fonction `fselect()`, bien que plus raide dans de nombreux cas, est aussi plus limitée. Il est donc utile de ma\^triser les deux.

```{r}
urchin1 <- fselect(urchin, origin, solid_parts, test) # Assignation normale
rmarkdown::paged_table(urchin1)
```
...
```{r}
urchin2 <- fselect(urchin, c(1, 4, 14))
rmarkdown::paged_table(urchin2)
```

Attention : la fonction `fselect()` n'est pas toujours utilisable pour remplacer la fonction `select()`. Par exemple, l'instruction `contains()` est utile pour sélectionner les valriables dont le nom contient une chaine de caractères. Mais cette forme n'est pas comprise par `fselect()`. De plus, avec `select()` il faut utiliser l'assignation alternative `%<-%` au lieu de `<-` pour "collecter" les résultats à la fin de traitement.
...
Idem pour `ends_with()` et d'autres operateurs du genre :
...
De même que toutes les colonnes d'un tableau ne sont pas forcément utiles, il est souvent nécessaire de sélectionner les lignes en fonction de critères particuliers pour restreindre l'analyse à une sous-population données, ou pour éliminer les cas qui ne correspondent pas à ce que vous voulez. `filter()` est une fonction tidy qui effectue ce travail. L'équivalent non-tidy porte un nom légèrement différent : `fsubset()`. Repartons du jeu de données `urchin_bio` simplifié à trois variables (`urchin2`).
...
... et le même en version non-tidy, l'assignation normale ne pose pas de problème ici :

```{r, echo = TRUE}
# Tous les oursins sauf ceux issus de la pêche
urchin_sub1 <- fsubset(urchin2, origin != "Fishery")
rmarkdown::paged_table(urchin_sub1)
```
...
urchin_sub2 <- fsubset(urchin2, height > 20)
...
urchin_sub3 <- fsubset(urchin2, height > 20 & origin == "Farm") 
...
La fonction tidy `mutate()` permet de calculer de nouvelles variables (si le nom fourni n'existe pas encore dans le jeu de donnée) ou écrase les variables existantes de même nom. La fonction non-tidy équivalente est `fmutate()`. Repartons du jeu de données `urchin`. Pour calculer de nouvelles variables, vous pouvez employer :
...
```{r, echo= TRUE}
urchin <- fmutate(urchin, 
  sum_skel  =  lantern + spines + test, 
  ratio     = sum_skel / skeleton,
  skeleton2 = skeleton^2)
rmarkdown::paged_table(collect(select(urchin, skeleton:spines, sum_skel:skeleton2)))
```
...
rmarkdown::paged_table(collect(select(urchin, skeleton, skeleton_log, skeleton_sqrt)))
```

La fonction tidy `transmute()` effectue la même opération, mais en plus, elle laisse tomber les variables d'origine pour ne garder *que* les nouvelles variables calculées. Il n'y as pas d'équivalent non-tidy.
...
La fonction tidy `group_by()` ou la fonction non-tidy `fgroup_by()` ne changnte rien dans le tableau lui-même, mais ajoutent une annotation qui indique que les calculs ultérieurs devront être effectués sur des sous-ensembles du tableau en parallèle. Ceci est surtout utile avec `summarise()`/`fsummarise()` (voir ci-dessous). Pour annuler le regroupement, il suffit d'utiliser `ungroup()`/`fungroup()`.

À noter que toutes les fonctions qui collectent les résultats, à savoir `as_dtx()`, `collect_dtx()` et les assignations alternatives `%<-%` et `%->%` dégroupent automatiquement les données. Ceci est voulu afin d'éviter que l'on oublie plus tard que l'objet a un regroupement enregistré et que vous ne réaliser par la suite des calculs non voulus (par groupes alors que vous pensez travailler individu par individu parce que vous avez oublié que vous aviez appliqué `group_by()`/`fgroup_by()` plus haut dans votre code). **Il faut toujours préciser les groupes aussi proche que possibnle de leur utilisation, et dégrouper éventuellement à la fin lorsqu'on n'en a plus besoin.**
...
Voici les mêmes calculs, mais effectués séparément pour les deux types de supplémentations alimentaires (ici, comme on utilise `%->%` à la fin du pipeline, on peut utiliser indifféremment le couple `fgroup_by()`/`fsummarise()`ou `group_by()`/`summarise()`. Aussi, on utilise l'opérateur de pipe "léger" `|>` au lieu de `%>.%` entre `(f)group_by()` et `(f)summarise()` sur la même ligne pour indiquer que ce n'est qu'une seule étape dans le calcul puisque `group_by()`/`fgroup_by()` ne fait en réalité aucun calcul) :

```{r}
tooth %>.%
  fgroup_by(., supp) |> fsummarise(
  "moyenne" = mean(len), 
  "minimum" = min(len), 
  "médiane" = median(len), 
  "maximum" = max(len)) %->%
...
-   Tout comme lors de réalisation d'une boite de dispersion, vous devez être particulièrement vigilant au nombre d'observation par sous-groupe. Pensez toujours à ajoutez à chaque tableau de résumé des données, le nombre d'observations par sous-groupe grâce à la fonction `n()`. Cependant, cette fonction n'est **pas** disponible avec `fgroup_by()` et `fsummarise()`. Vous *devez* donc absolument utiliser `group_by()` et `summarise()` icic, sinon, vous aurez une erreur !

```{r}
tooth %>.% 
  group_by(., supp) |> summarise(
  "moyenne" = mean(len), 
  "minimum" = min(len), 
  "médiane" = median(len), 
  "maximum" = max(len),
...
Toutefois, `fsummarise()` fonctionne différemment et n'a pas de problèmes dans ce cas-là:

```{r}
tooth_summary4 <- fsummarise(tooth,
  "len" = mean(len), # Notez le même nom à gauche et à droite du = (len)
  "len_sd" = sd(len))
...
Faites bien attention : les couples de fonctions avec et sans "f" comme `select()`/`fselect()`, `mutate()`/`fmutate()` ou `summarise()`/`fsummarise()` se ressemblent très fort au niveau de l'interface. On pourrait donc être tenté de croire qu'elles sont parfaitement interchangeables. La plupart du temps, c'est le cas. Cependnant, elles fonctionnement radicalement différemment en interne et des différences existent, sont connues, et ne sont *pas* des bugs. Vérifiez toujours votre code !
...
biometry_tab %<-% summarise(biometry_25, # %<-% au lieu de <- en dernière étape
  mean   = mean(bmi), 
  median = median(bmi),
  number = n())
...
Pensez toujours à assigner le résultat de la dernière étape de votre pipeline avec l'opérateur `%->%` qui va dégrouper les données (s'il y avait un `group_by()` dans le pipeline). Cela évite des problèmes plus tard, si on oublie que le regroupement est là ! Il va aussi collecter les résultats du pipeline, c'est-à-dire effectuer l'équivalent de `collect_dtx()`, en un objet data frame dans sa forme par défaut (classe **data.frame**, **data.table** ou **tbl_bf**). Si vous n'avez pas modifié les options de `SciViews::R`, cette classe sera un **data.table**.
